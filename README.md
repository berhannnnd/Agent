...
- SQLite persistence for run records, runtime events, checkpoints, and approval audit.
...
agent/
  ...
  security/       Tool permission rules, approval audit, future safety boundaries
...
To persist runs, runtime events, approval checkpoints, and approval audit in one local database:
...
Each chat request creates a new run. `run_id` is generated by the gateway run service and passed into `AgentSession.send()` or `AgentSession.stream()`. Runtime checkpoints and gateway run records share that same ID. In file mode, each run is stored as one JSON document under `AGENT_RUN_ROOT`. In SQLite mode, runs, runtime events, checkpoints, and approval audit records share `AGENT_DB_PATH`.
...
- Add new run persistence backends by implementing `agent.runs.RunStore`; checkpoint and approval audit storage should stay behind their own store interfaces. Gateway should only choose and call adapters.
