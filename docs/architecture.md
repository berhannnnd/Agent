...
- `agent`: 智能体系统核心。按 runtime、context、models、tools 主链路组织，并用 `state` 收束长期状态，用 `governance` 收束权限、凭证、审计和追踪。该层不依赖 FastAPI。
...
6. `agent.integrations` 装配 skills/MCP；`agent.state.workspaces` 根据 `AgentSpec.workspace` 解析 workspace。
...
5. 新增长期数据域：按归属放入 `agent.state.identity`、`agent.definitions`、`agent.state.workspaces`、`agent.memory` 或 `agent.governance`，并接入 `gateway.services.GatewayPersistence`。
